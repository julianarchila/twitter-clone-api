...
class Tweet(TwModel):
    """Tweet model """
    user = models.ForeignKey("users.User",related_name="tweets" ,on_delete=models.CASCADE)
    parent = models.ForeignKey("self", null=True, blank=True, on_delete=models.SET_NULL)
    content = models.TextField(blank=True, null=True)
    image = models.ImageField(upload_to="tweets/",null=True, blank=True)
    likes = models.ManyToManyField("users.User", related_name="liked_tweets",blank=True, through=TweetLike)
